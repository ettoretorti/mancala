...
uint8_t MiniMaxAgent::makeMove(const Board& b, Side s, size_t movesSoFar, uint8_t lastMove) {
	...
	Board bCopy = b;
	return moves[minimax_alphabeta(depth, s, s, bCopy, movesSoFar, -1.0, 96.0).first];
}
...
static std::pair<uint8_t,double> minimax_alphabeta(uint8_t depth, Side s, Side yourSide, Board& b, size_t movesSoFar, double alpha, double beta){
	...
	if(toMove == yourSide){
		...
		if(movesSoFar == 1){
			...
			std::pair<uint8_t, double> currentMove = minimax_alphabeta(depth-1, toMove, yourSide, bCopy, movesSoFar+1, alpha, beta);
